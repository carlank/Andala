...
class Canvas {
  ...
  setUserInputListeners(){
    ...
    const brushColorInput = document.querySelector('#brush-color-input');
    brushColorInput.addEventListener('change', event => {
      brushColorInput.dataset.color = event.target.value;
      this.toggleBrushColor(event);
    })
  ...
  determineDraw(e){
    if (this.drawing){
      ...
      const isCelticKnot = document.getElementById('knot-input').checked;
